...
export function TableProvider({ children }) {
  ...
  const addSkill = (e, index, skillName, skillValue) => {
    e.preventDefault();
    if (!(skillName in usersData[index].skills) && skillName) {
  ...
  return <TableContext.Provider value={{ updateUsers, salarySum, skillNames, sumSalary, sumUserSkills, usersData, deleteUser, deleteAllUsers, sortBySalary, editUser, editSkills, deleteSkill, addSkill }}>{children}</TableContext.Provider>;
}
